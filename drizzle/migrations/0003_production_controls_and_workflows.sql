...
CREATE OR REPLACE FUNCTION public.prevent_invoice_delete_with_payments()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
BEGIN
  IF EXISTS (SELECT 1 FROM public.payments WHERE invoice_id = OLD.id AND voided_at IS NULL) THEN
    RAISE EXCEPTION 'Paid invoices cannot be deleted; void or reverse their payments first';
  END IF;
  RETURN OLD;
END;
$$;
DROP TRIGGER IF EXISTS invoices_prevent_delete_with_payments ON public.invoices;
CREATE TRIGGER invoices_prevent_delete_with_payments
  BEFORE DELETE ON public.invoices FOR EACH ROW
  EXECUTE FUNCTION public.prevent_invoice_delete_with_payments();
...
DECLARE target_id UUID; item JSONB; saved_invoice public.invoices%ROWTYPE;
BEGIN
  IF _id IS NULL THEN
    INSERT INTO public.invoices (invoice_number, client_id, project_id, issue_date, due_date, discount, tax, status, notes, subtotal)
    VALUES ('', _client_id, _project_id, _issue_date, _due_date, COALESCE(_discount, 0), COALESCE(_tax, 0), _status, _notes, 0)
    RETURNING id INTO target_id;
  ELSE
    UPDATE public.invoices
    SET client_id = _client_id, project_id = _project_id, issue_date = _issue_date,
        due_date = _due_date, discount = COALESCE(_discount, 0), tax = COALESCE(_tax, 0),
        status = _status, notes = _notes
    WHERE id = _id
    RETURNING id INTO target_id;
    IF target_id IS NULL THEN RAISE EXCEPTION 'Invoice not found'; END IF;
...
END;
$$;
GRANT EXECUTE ON FUNCTION public.save_invoice(UUID, UUID, UUID, DATE, DATE, public.invoice_status, NUMERIC, NUMERIC, TEXT, JSONB) TO authenticated;
