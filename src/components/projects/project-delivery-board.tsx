...
export function ProjectDeliveryBoard({ projectId }: { projectId: string }) {
  ...
  const addMilestone = useMutation({
    mutationFn: async () => {
      const { error } = await supabase
        .from("project_milestones" as never)
        .insert({ project_id: projectId, title: title.trim() } as never);
      if (error) throw error;
    },
    onSuccess: () => {
      setTitle("");
  ...
  const addTask = useMutation({
    mutationFn: async () => {
      const { error } = await supabase
        .from("project_tasks" as never)
        .insert({ project_id: projectId, title: taskTitle.trim() } as never);
      if (error) throw error;
    },
    onSuccess: () => {
      setTaskTitle("");
  ...
  const uploadFile = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from("project_files" as never).insert({
      ...
      <CardContent className="space-y-6">
        <div className="grid gap-3 sm:grid-cols-2">
          <div className="flex gap-2">
            ...
            />
            <Button
              disabled={!title.trim() || addMilestone.isPending}
          ...
          <section className="space-y-2">
            ...
            {milestonesQuery.data?.map((item) => (
              <div key={item.id} className="flex items-center gap-2 rounded-md border p-3">
                ...
                <Select
                  value={item.status}
                  onValueChange={(status) => updateMilestone.mutate({ id: item.id, status })}
                >
                ...
                <Button
                  ...
                  onClick={() => deleteItem.mutate({ table: "project_milestones", id: item.id })}
                >
          ...
          <section className="space-y-2">
            ...
            {tasksQuery.data?.map((item) => (
              <div key={item.id} className="flex items-center gap-2 rounded-md border p-3">
                ...
                <Select
                  value={item.status}
                  onValueChange={(status) => updateTask.mutate({ id: item.id, status })}
                >
                ...
                <Button
                  ...
                  onClick={() => deleteItem.mutate({ table: "project_tasks", id: item.id })}
                >
                  <Trash2 className="h-4 w-4 text-red-600" />
                </Button>
              </div>
        ...
        <section className="space-y-3">
          ...
          {filesQuery.data?.map((file) => (
            <div key={file.id} className="flex items-center gap-2 rounded-md border p-3 text-sm">
          ...
          {filesQuery.data?.length === 0 && (
            <p className="text-sm text-muted-foreground">No project files uploaded.</p>
          )}
        </section>
      </CardContent>
    </Card>
